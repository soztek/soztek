...
import { Star, Truck, ShieldCheck, CreditCard, Headset } from "lucide-react";
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            {product.brand && (
              <span className="rounded-md bg-green-800 px-2.5 py-1 text-xs font-bold uppercase tracking-wide text-white">
                {product.brand}
              </span>
            )}
          ...
          {product.sku && <p className="mt-1 text-sm text-ink/50">Ürün Kodu: {product.sku}</p>}
          ...
          <div className="mt-5 flex items-baseline gap-3">
            <span className="text-4xl font-extrabold text-green-800">{formatPrice(dto.price)}</span>
            {dto.compareAt && (
          ...
          <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              { i: CreditCard, t: "Taksit İmkânı" },
              { i: Headset, t: "Teknik Destek" },
            ].map((b, idx) => (
